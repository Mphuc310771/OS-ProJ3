...
int keys[NKEYS];
int nthread = 1;
pthread_mutex_t locks[NBUCKET]; // One lock per bucket for fine-grained locking
...
static void put(int key, int value) {
  ...
  pthread_mutex_lock(&locks[i]); // Lock this bucket
  ...
  pthread_mutex_unlock(&locks[i]); // Unlock this bucket
}
